---
title: Quickstart
description: Get started with cereal in minutes
author: zoobzio
...
func main() {
    ...
    // Create cereal instance with PostgreSQL provider
    users, err := cereal.New[User](db, "users", postgres.New())
    if err != nil {
        log.Fatal(err)
    }
...
Cereal supports multiple databases through ASTQL providers:
...
users, err := cereal.New[User](db, "users", mysql.New())   // MySQL
users, err := cereal.New[User](db, "users", sqlite.New()) // SQLite
users, err := cereal.New[User](db, "users", mssql.New())  // MSSQL
```

## Struct Tags

Cereal uses struct tags to define the schema:
