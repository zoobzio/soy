---
title: Overview
description: Type-safe SQL query builder for Go with multi-database support
author: zoobzio
...
// Create a cereal instance with a database provider
...
users, err := cereal.New[User](db, "users", postgres.New())
users, err := cereal.New[User](db, "users", mysql.New())
users, err := cereal.New[User](db, "users", sqlite.New())
users, err := cereal.New[User](db, "users", mssql.New())
```
