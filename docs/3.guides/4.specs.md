...
```go
condition := cereal.ConditionSpec{
    Field:    "age",
    Operator: ">=",
    Param:    "min_age",
}
...
countSpec := cereal.AggregateSpec{
    Where: []cereal.ConditionSpec{
        {Field: "status", Operator: "=", Param: "status"},
    },
}
