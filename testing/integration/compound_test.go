...
func TestCompoundFromSpec_Integration(t *testing.T) {
	tdb := setupTestDB(t)
	defer tdb.cleanup(t)
	createTestTable(t, tdb.db)
	...
	t.Run("compound from spec execution", func(t *testing.T) {
		limit := 10
			...
			Operands: []cereal.SetOperandSpec{
				{
					...
					Query: cereal.QuerySpec{
						Where: []cereal.ConditionSpec{
							{Field: "name", Operator: "=", Param: "target_name"},
						},
					},
				},
			},
		...
		compound := c.CompoundFromSpec(spec)
		users, err := compound.Exec(ctx, map[string]any{
			"q0_min_age":     30,
			"q1_target_name": "Alice",
		})
		if err != nil {
			t.Fatalf("CompoundFromSpec().Exec() failed: %v", err)
		}

		// Should get Alice (25, by name), Bob (30), Charlie (35)
		if len(users) != 3 {
	...
	t.Run("compound from spec with transaction", func(t *testing.T) {
			...
			Operands: []cereal.SetOperandSpec{
				{
					...
					Query: cereal.QuerySpec{
						Where: []cereal.ConditionSpec{
							{Field: "age", Operator: ">=", Param: "exclude_age"},
						},
					},
				},
			},
		...
		compound := c.CompoundFromSpec(spec)
		users, err := compound.ExecTx(ctx, tx, map[string]any{
			"q0_min_age":     25,
			"q1_exclude_age": 35,
		})
		if err != nil {
			t.Fatalf("CompoundFromSpec().ExecTx() failed: %v", err)
		}

		// age >= 25: Alice(25), Bob(30), Charlie(35)
		// age >= 35: Charlie(35)
		// Except: Alice(25), Bob(30)
		if len(users) != 2 {
